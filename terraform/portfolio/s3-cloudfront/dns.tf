data "aws_route53_zone" "stuchlik_dev" {
  name         = var.domain_name
  private_zone = false
}

# validation DNS records
resource "aws_route53_record" "dns_validation" {
  for_each = {
    for dvo in aws_acm_certificate.portfolio.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
  }

  allow_overwrite = true
  name            = each.value.name
  records         = [each.value.record]
  ttl             = 60
  type            = each.value.type
  zone_id         = data.aws_route53_zone.stuchlik_dev.id
}

# actual DNS for the distribution
resource "aws_route53_record" "portfolio" {
  for_each = aws_cloudfront_distribution.web_distribution.aliases
  zone_id  = data.aws_route53_zone.stuchlik_dev.id
  name     = each.value
  type     = "A"

  alias {
    name                   = aws_cloudfront_distribution.web_distribution.domain_name
    zone_id                = aws_cloudfront_distribution.web_distribution.hosted_zone_id
    evaluate_target_health = false
  }
}